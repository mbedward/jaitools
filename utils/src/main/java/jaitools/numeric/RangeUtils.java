 ...
 * @since 1.1
 * @version $Id: RangeUtils.java 1142 2010-03-25 11:09:06Z michael.bedward $
 */
public class RangeUtils {
	
    public static <T extends Number & Comparable> List<Range<T>> createComplement(List<Range<T>> ranges) {
    	List<Range<T>> complements = new ArrayList<Range<T>>();
    	if (ranges != null && !ranges.isEmpty()){
    		final int size = ranges.size();
    		if (size == 1) {
    			return createComplement(ranges.get(0));
    		} else {
    			for (int i = 0; i< size-1;i++){
    				Range<T> left = ranges.get(i);
    				Range<T> right = ranges.get(i+1);
    				if (i==0) {
    					if (left.isMinClosed()) {
    		                complements.add(Range.create(null, true, left.getMin(), !left.isMinIncluded()));
    					}
    				}
    					
    				complements.add(Range.create(left.getMax(), !left.isMaxIncluded(), right.getMin(), !right.isMinIncluded()));
    				if (i == size - 2) {
    					if (right.isMaxClosed()) {
    						complements.add(Range.create(right.getMax(), !right.isMaxIncluded(), null, true));
    					}
    				}
